...
Simulator_aTAM::Simulator_aTAM(AssemblyTile * seedTile, SetOfAssemblyTiles *S, QMap<int, int> &StrengthFunction, int Theta, int StepNumber, QMap<int, QColor> &C)
    : manager(S), StrengthMap(StrengthFunction), ThetaParameter(Theta), NumberOfSteps(StepNumber), ColorMap(C)
{
    //0s set of Assembly Tiles is always a set of building block tiles
    //create first set of seed tiles
    SetOfAssemblyTiles * seed = new SetOfAssemblyTiles(seedTile);
    manager.addSet(seed); //add it to the manager
}

void Simulator_aTAM::initialize()
/*
...
void Simulator_aTAM::startSimulation()
/*
...
#endif
            SetOfAssemblyTiles &temp = this->manager.getAssemblyTileSet(0);   //get building block set of Assembly tiles
#ifdef DEBUG
...
SetOfAssemblyTiles & Simulator_aTAM::selectMostCurrentSetOfAssemblyTiles()
/*
 ...
 */
{
    return manager.getAssemblyTileSet(currentStep);
}

QList<FitPlace*> *Simulator_aTAM::findFittingSpots(AssemblyTile &T1,AssemblyTile &T2)
/*
...
AssemblyTile * Simulator_aTAM::attemptToCombine(AssemblyTile *T1,AssemblyTile *T2, FitPlace *place)
/*
 ...
 */
{
    ...
    if(checkStrength(*T1, *T2, &boundaryList)) //check if two tiles  have enough bond strength
    {
#ifdef DEBUG
...
bool Simulator_aTAM::checkStrength(AssemblyTile & T1, AssemblyTile & T2, QList<boundaryPoint*> *boundary)
/*
 Post-Condition:  Also construct a list of boundary points, and check bond strength
 */
{
#ifdef DEBUG
    qDebug()<<"-------------------------------------------------------------------------------------------------";
...
int Simulator_aTAM::getBondStrength(ActiveTile & t1, ActiveTile & t2, direction boundary)
/*
...
void Simulator_aTAM::tileModificationFunction(AssemblyTile & T, QList<boundaryPoint *> *boundary)
/*
...
QList<DisplayTile*> Simulator_aTAM::toDisplayTile(AssemblyTile * T)
{
...
QList<SetOfAssemblyTiles *> &Simulator_aTAM::getAssemblies()
